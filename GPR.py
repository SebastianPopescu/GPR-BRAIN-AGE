...
class GaussianProcessRegression(object):
	...
	def __init__(self,num_data,num_test,dim_input,dim_output,lista_initializare,age_mean,num_validation,num_iterations):
...
if __name__ == '__main__':
	...
	parser.add_argument('--input_feature_path',type=str,help='the absolute path of the file which contains your features dataset')
	parser.add_argument('--input_age_path',type=str,help='the absolute path of the file which contains your chrnonological age columns-vector')
	parser.add_argument('--num_iterations',type=int,default=1000,help='the number of iterations in the training process') 
	...
	t0 = time.time()
	Y_total = np.genfromtxt(args.input_feature_path,dtype=np.float64)
	Y_total = Y_total.reshape((2001,1))
	print('shape of Y')
	print(Y_total.shape)
	X_total = np.genfromtxt(args.input_age_path,dtype=np.float64)
	print('shape of X')
	print(X_total.shape)
	np.random.seed(7)
	...
	t4=time.time()
	obiect =  GaussianProcessRegression(num_data = X_training.shape[0],num_test=X_testing.shape[0],dim_input=X_training.shape[1],dim_output=Y_training.shape[1],lista_initializare=lista_initializare,age_mean=age_mean,num_validation=X_validation.shape[0],num_iterations=num_iterations)
